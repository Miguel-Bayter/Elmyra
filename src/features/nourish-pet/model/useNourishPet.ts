...
export const useNourishPet = () => {
  ...
  const isDisabled =
    !companion || companion.isResting || companion.isInRestMode || companion.nourishment >= 95;

  const nourish = useCallback(() => {
    const now = Date.now();
    // SECURITY: Debounce prevents rapid stat manipulation (R6)
    if (now - lastActionRef.current < ACTION_DEBOUNCE_MS) return;
    if (isDisabled) return;
    lastActionRef.current = now;
    performAction('nourish');
  }, [isDisabled, performAction]);

  return { nourish, isDisabled };
};
